...
class JumboConnector:
    ...
    def get_product_details(self, product):
        ...
        response = requests.get(
            'https://mobileapi.jumbo.com/v14/products/{}'.format(product_id),
            headers=HEADERS
        )
        if not response.ok:
            response.raise_for_status()
    ...
    def get_categories(self):
        response = requests.get(
            'https://mobileapi.jumbo.com/v14/categories',
            headers=HEADERS
        )
        if not response.ok:
            response.raise_for_status()
    ...
    def get_all_stores(self):
        response = requests.get(
            'https://mobileapi.jumbo.com/v14/stores',
            headers=HEADERS
        )
        if not response.ok:
            response.raise_for_status()
    ...
    def get_store(self, store):
        ...
        response = requests.get(
            'https://mobileapi.jumbo.com/v14/stores/{}'.format(store_id),
            headers=HEADERS
        )
        if not response.ok:
            response.raise_for_status()
    ...
    def get_promotions_store(self, store):
        store_id = store if not isinstance(store, dict) else store['id']
        response = requests.get(
            'https://mobileapi.jumbo.com/v14/promotion-overview',
            headers=HEADERS,
